...
type ProxyNodeSource = 'ollama-proxy' | 'lmstudio-proxy' | 'lemonade-proxy'
 ...
 */
export type ProxyEngine = Extract<EngineType, 'ollama' | 'lm-studio' | 'lemonade'>
export const PROXY_ENGINES: readonly ProxyEngine[] = ['ollama', 'lm-studio', 'lemonade']
...
function emptyEngines(): Record<ProxyEngine, EnginePresence> {
    return { ollama: emptyPresence(), 'lm-studio': emptyPresence(), lemonade: emptyPresence() }
}
...
function setEngine(
    ...
    return {
        ...
        lemonade: engine === 'lemonade' ? presence : engines.lemonade
    }
}
...
export function isProxyEngine(engine: EngineType): engine is ProxyEngine {
    return engine === 'ollama' || engine === 'lm-studio' || engine === 'lemonade'
}
...
class ModularBridgeState {
    ...
    private proxyPorts: Record<ProxyEngine, number> = { ollama: 0, 'lm-studio': 0, lemonade: 0 }
    ...
    handleNotification(notification: JsonRpcNotification): void {
        ...
        if (notification.source === 'lemonade-proxy') {
            this.handleProxyNotification(notification, 'lemonade')
            return
        }
