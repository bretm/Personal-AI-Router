...
On Linux, `nvpair-ui-broker --headless` keeps the worker tree running without a
desktop or terminal controller and shuts it down cleanly on `SIGTERM`. Use the
provided systemd user-unit installer rather than keeping a terminal session
open. See [Running PAIR as a systemd user service](systemd-service.mdx) for the
installation, boot, update, and operating procedure.
