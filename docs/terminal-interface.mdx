...
For an unattended Linux node that does not need an interactive controller, use
the [systemd user service](systemd-service.mdx) instead. Stop that service before
launching the TUI; both modes own a complete broker and worker tree.
...
| 9 | **Auth** | Owner-managed PAIR client tokens and this node's engine credentials |
...
On **Auth** (9), the founding node can initialize cluster authentication (`b`),
create a client token (`n`), and revoke the selected client (`r`). A newly
created token is displayed once; save it outside PAIR, then press `x` to dismiss
it. Other paired nodes show validator status but cannot list or change clients.

Each node manages its own engine credential independently. Press `e` to enter a
reference such as `engine.example.api_key`, `s` to set its write-only value, or
`c` to clear it. PAIR shows only whether the value is configured; it never
