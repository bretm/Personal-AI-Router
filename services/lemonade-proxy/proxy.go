...
// which for streaming Lemonade responses is dominated by token
...
// non-inference noise. Lemonade serves the OpenAI-compatible API, so these
...
func (p *Proxy) Run(ctx context.Context) error {
	...
	// connection — so the target Lemonade sees the client disconnect and stops
...
func (retrySignal) Error() string { return "lemonade-proxy: retry next candidate" }
...
// (self-forward guard): a local Lemonade advertisement could otherwise
...
func upstreamUnreachableID(nodeID string) string {
	return "lemonade-proxy:upstream-unreachable:" + nodeID
}
