...
- **Linux** (first-class): NVIDIA GPU inventory, dedicated VRAM usage, and utilization come from `nvidia-smi`. AMD GPU counters come from the kernel's DRM sysfs interface, joined by PCI address to AMD-SMI identity when available; AMD-SMI remains a field-level fallback, and `ghw` supplies the name when it is absent. CPU and system-memory usage come from `/proc`. NVIDIA unified-memory GPUs use the `/proc/meminfo` system-memory snapshot even when dynamic `nvidia-smi` collection is unavailable. KFD topology identifies AMD APUs such as Strix Halo, which report their GTT pool as GPU capacity and use; discrete Radeons retain dedicated VRAM even if they expose a large GTT aperture.
