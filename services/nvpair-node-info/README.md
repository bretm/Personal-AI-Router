...
- `vram_bytes` is reported through DXGI on Windows, NVIDIA SMI or AMD SMI on Linux, and IORegistry on macOS. On a unified-memory NVIDIA GPU such as DGX Spark, Linux uses total physical system memory for `vram_bytes` and the independently sampled system-memory usage for `vram_used_bytes`. On an AMD APU such as Strix Halo, AMD SMI identifies system DDR graphics memory and Linux reports its GTT shared-GPU pool and used amount. On Apple Silicon, `vram_bytes` is total physical unified memory and `vram_used_bytes` is the GPU driver's mapped allocation (`Alloc system memory`), not whole-system RAM usage or the momentarily active subset.
...
- **Linux** (first-class): NVIDIA GPU inventory, dedicated VRAM usage, and utilization come from `nvidia-smi`; AMD GPU identity, memory, and utilization come from `amd-smi`; CPU and system-memory usage come from `/proc`. The two vendor tools are sampled independently, so a missing or broken one does not suppress the other. NVIDIA unified-memory GPUs use the `/proc/meminfo` system-memory snapshot even when dynamic `nvidia-smi` collection is unavailable. AMD APUs whose AMD SMI memory type is system DDR, including Strix Halo, report their GTT pool as GPU capacity and use; discrete GDDR/HBM Radeons retain dedicated VRAM even if they expose a GTT aperture. When neither vendor tool identifies a GPU, `ghw` supplies adapter names without dynamic GPU stats.
