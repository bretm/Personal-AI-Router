...
V_LEMONADEPROXY=$(jq -r --arg k 'lemonade-proxy' '.components[$k]' "$VERSIONS_FILE")
...
build_subbinary 3 lemonade-proxy    "$V_LEMONADEPROXY"
...
cp "$ROOT/lemonade-proxy/lemonade-proxy"     "$BIN_OUT/lemonade-proxy"
